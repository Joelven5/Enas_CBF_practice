...
class SignalLogger:
    ...
    def enable_auto_logging(enable: bool) -> StatusCode:
        ...
        Auto logging is enabled by default on the roboRIO and disabled by default on
        other systems. Additionally, on a roboRIO 1, auto logging will only be active
        if a USB flash drive is present.

        When auto logging is enabled, logging is started by any of the following
        (whichever occurs first):

        - It has been at least 1 second since program startup (allowing for calls
          to self.set_path), and the robot is enabled.
        - It has been at least 5 seconds since program startup (allowing for calls
          to self.set_path), and the Driver Station is connected to the robot (if on
          a roboRIO).
    ...
    if USE_WPILIB:
        __struct_array_type_names: dict[str, str] = {}
        ...
        def write_struct(cls, name: str, struct: type[T], value: T, latency_seconds: second = 0) -> StatusCode:
            ...
            return cls.write_schema_value(name, type_name, HootSchemaType.STRUCT, wpistruct.pack(value), latency_seconds)
        ...
        def write_struct_array(cls, name: str, struct: type[T], values: list[T], latency_seconds: second = 0) -> StatusCode:
            ...
            struct_type_name = wpistruct.getTypeName(struct)
            if struct_type_name in cls.__struct_array_type_names:
                type_name = cls.__struct_array_type_names[struct_type_name]
            else:
                type_name = struct_type_name + "[]"
                cls.__struct_array_type_names[struct_type_name] = type_name
