"""
Contains the common base class for status signals.
...
class BaseStatusSignal(ABC):
    ...
    def status(self) -> StatusCode:
        """
        The status code of the last time this signal was refreshed.

        :return: Status code of the last time this signal was refreshed
        :rtype: StatusCode
        """
        return self._status
    ...
    def set_update_frequency(self, frequency_hz: hertz, timeout_seconds: second = 0.100) -> StatusCode:
        ...
        retval = StatusCode(Native.instance().c_ctre_phoenix6_SetUpdateFrequency(0, network, self.__device_identifier.device_hash, self.__spn, frequency_hz, timeout_seconds))
        if not retval.is_ok():
            device = str(self.__device_identifier) + " set_update_frequency " + self.__name
            report_status_code(retval, device)
        return retval
    ...
    @staticmethod
    def wait_for_all(timeout_seconds: second, *signals: 'BaseStatusSignal | list[BaseStatusSignal]', report_error: bool = True) -> StatusCode:
        """
        ...
        :param report_error: Whether to report any errors to the Driver Station/stderr. This must
                             be specified using a named parameter at the end of the parameter list.
        :type report_error: bool, optional
        :returns:   InvalidNetwork if signals are on different CAN bus networks,
        ...
        :rtype: StatusCode
        """
        return BaseStatusSignal.__wait_for_all_impl("BaseStatusSignal.wait_for_all", timeout_seconds, report_error, BaseStatusSignal.__flatten_signals(*signals))

    @staticmethod
    def refresh_all(*signals: 'BaseStatusSignal | list[BaseStatusSignal]', report_error: bool = True) -> StatusCode:
        """
        ...
        :rtype: StatusCode
        """
        return BaseStatusSignal.__wait_for_all_impl("BaseStatusSignal.refresh_all", 0, report_error, BaseStatusSignal.__flatten_signals(*signals))
    ...
    def set_update_frequency_for_all(frequency_hz: hertz, *signals: 'BaseStatusSignal | list[BaseStatusSignal]') -> StatusCode:
        ...
        retval = StatusCode(Native.instance().c_ctre_phoenix6_SetUpdateFrequencyForAll(0, sigs, count, frequency_hz, 0.100))
        if not retval.is_ok():
            report_status_code(retval, "BaseStatusSignal.set_update_frequency_for_all")
        return retval
    ...
    @staticmethod
    def __wait_for_all_impl(location: str, timeout_seconds: second, report_error: bool, signals: list['BaseStatusSignal']) -> StatusCode:
        count = len(signals)
        if count == 0:
            return StatusCode.OK

        network = signals[0].__device_identifier.network
        for signal in signals[1:]:
            if signal.__device_identifier.network != network:
        ...
        status = StatusCode(Native.instance().c_ctre_phoenix6_get_signal(count, sigs, rets, ctypes.c_char_p(bytes(network.name, 'utf-8')), True, timeout_seconds))
        ...
        # error reporting
        if report_error and not status.is_ok():
            for signal in signals:
