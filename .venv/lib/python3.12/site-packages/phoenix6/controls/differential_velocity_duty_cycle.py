"""
Contains the class for controlling a device using DifferentialVelocityDutyCycle.
...
class DifferentialVelocityDutyCycle:
    ...
    def _send_request(self, network: CANBus, device_hash: int) -> StatusCode:
        ...
        :rtype: StatusCode
        """
        return StatusCode(Native.instance().c_ctre_phoenix6_RequestControlDifferentialVelocityDutyCycle(ctypes.c_char_p(bytes(network.name, 'utf-8')), device_hash, self.update_freq_hz, self.average_velocity, self.differential_position, self.enable_foc, self.average_slot, self.differential_slot, self.override_brake_dur_neutral, self.limit_forward_motion, self.limit_reverse_motion, self.ignore_hardware_limits, self.ignore_software_limits, self.use_timesync))
