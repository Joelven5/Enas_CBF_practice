"""
Contains the class for controlling a device using RainbowAnimation.
...
class RainbowAnimation:
    ...
    def _send_request(self, network: CANBus, device_hash: int) -> StatusCode:
        ...
        :rtype: StatusCode
        """
        return StatusCode(Native.instance().c_ctre_phoenix6_RequestControlRainbowAnimation(ctypes.c_char_p(bytes(network.name, 'utf-8')), device_hash, self.update_freq_hz, self.led_start_index, self.led_end_index, self.slot, self.brightness, self.direction.value, self.frame_rate))
