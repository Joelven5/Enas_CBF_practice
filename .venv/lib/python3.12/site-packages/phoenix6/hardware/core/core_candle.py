...
class CoreCANdle(ParentDevice):
    ...
    def none(cls):
        """
        Constructs a stubbed-out CoreCANdle, where all status signals, controls, configs,
        ...
        :returns: Stubbed-out CoreCANdle
        :rtype: CoreCANdle
        """
        return cls(-1, CANBus())
    ...
    @final
    def set_control(self, request: SupportsSendRequest) -> StatusCode:
        if isinstance(request, (ModulateVBatOut, SolidColor, EmptyAnimation, ColorFlowAnimation, FireAnimation, LarsonAnimation, RainbowAnimation, RgbFadeAnimation, SingleFadeAnimation, StrobeAnimation, TwinkleAnimation, TwinkleOffAnimation, EmptyControl)):
