"""
Contains the class for CTR Electronics' CANdle® branded device, a device that
controls LEDs over the CAN bus.
...
try:
    ...
    class CANdle(CoreCANdle, Sendable):
        ...
        def none(cls):
            """
            Constructs a stubbed-out CANdle, where all status signals, controls, configs,
        ...
        def clear_all_animations(self) -> StatusCode:
            """
            Clears all animations on the CANdle. This is equivalent to applying
            an EmptyAnimation to all the animation slots on the CANdle.

            :returns: Status code of the first failed control call, or OK if
                      all succeeded
            :rtype: StatusCode
            """
            retval = StatusCode.OK

            empty = EmptyAnimation(0)
            for i in range(0, self.ANIM_SLOT_COUNT):
                status = self.set_control(empty.with_slot(i))
                if retval.is_ok():
                    retval = status
