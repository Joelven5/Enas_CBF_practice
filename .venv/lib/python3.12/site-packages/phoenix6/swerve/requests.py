"""
Contains swerve requests used to control the swerve drivetrain.
...
class TargetDirectionPerspectiveValue(Enum):
    """
    In field-centric control, the direction of "forward" for the target direction sometimes
    differs from "forward" for translation. This addresses which forward to use.
    """

    USE_FORWARD_PERSPECTIVE = 0
    """
    Match the forward_perspective parameter for heading control.
    """
    BLUE_ALLIANCE = 1
    """
    "Forward" (0 deg) is always from the perspective of the blue alliance (i.e.
    towards the red alliance). This can be used to allow for the robot rotation to track
    vision targets while using operator-perspective joystick control for translation.
    """
...
class FieldCentricFacingAngle(SwerveRequest):
    ...
    def __init__(self):
        ...
        The perspective to use when determining which direction is forward.
        """
        self.target_direction_perspective: TargetDirectionPerspectiveValue = TargetDirectionPerspectiveValue.USE_FORWARD_PERSPECTIVE
        """
        The perspective to use when determining which direction is forward for the
        self.target_direction. This allows for the heading control to use a different
        perspective from the translation control.
        """
    ...
    def apply(self, parameters: 'SwerveControlParameters', modules_to_apply: list[SwerveModule]) -> StatusCode:
        ...
        if (
            self.target_direction_perspective is TargetDirectionPerspectiveValue.USE_FORWARD_PERSPECTIVE and
            self.forward_perspective is ForwardPerspectiveValue.OPERATOR_PERSPECTIVE
        ):
    ...
    def with_target_direction_perspective(self, new_target_direction_perspective: TargetDirectionPerspectiveValue) -> 'FieldCentricFacingAngle':
        """
        Modifies the target_direction_perspective parameter and returns itself.

        The perspective to use when determining which direction is forward for the
        self.target_direction. This allows for the heading control to use a different
        perspective from the translation control.

        :param new_target_direction_perspective: Parameter to modify
        :type new_target_direction_perspective: TargetDirectionPerspectiveValue
        :returns: this object
        :rtype: FieldCentricFacingAngle
        """

        self.target_direction_perspective = new_target_direction_perspective
        return self
