"""
Contains the main swerve drivetrain class.
...
class SwerveDrivetrain(Generic[DriveMotorT, SteerMotorT, EncoderT]):
    ...
    @overload
    def optimize_bus_utilization(self) -> StatusCode:
        """
        Optimizes the bus utilization of all devices in the swerve drivetrain by
        reducing the update frequencies of their status signals. All signals necessary
        for drivetrain functionality will remain enabled.

        This is equivalent to calling ParentDevice.optimize_bus_utilization_for_all
        with all devices in the drivetrain.
    ...
    @overload
    def optimize_bus_utilization(self, optimized_freq_hz: hertz) -> StatusCode:
        """
        Optimizes the bus utilization of all devices in the swerve drivetrain by
        reducing the update frequencies of their status signals. All signals necessary
        for drivetrain functionality will remain enabled.
        ...
        :param optimized_freq_hz: The update frequency to apply to the
                                  ...
                                  frequency is 4 Hz. This must be specified
                                  using a named parameter at the end of the
                                  parameter list.
        :type optimized_freq_hz: hertz
        :returns: Status code of the first failed optimize call,
                  or OK if all succeeded
        :rtype: StatusCode
        """
    ...
    def optimize_bus_utilization(self, optimized_freq_hz: hertz | None = None) -> StatusCode:
        devices: list[CommonDevice] = [self.pigeon2]
        devices.extend(
            dev
            for module in self.__modules
            for dev in (module.drive_motor, module.steer_motor, module.encoder)
        )
        if optimized_freq_hz is not None:
            return ParentDevice.optimize_bus_utilization_for_all(devices, optimized_freq_hz=optimized_freq_hz)
        else:
            return ParentDevice.optimize_bus_utilization_for_all(devices)
